...
def Step2_ComputeDist(img, Point1, Point2, BlockSize):
    ...
    Block1 = (
        img[Point1[0] : Point1[0] + BlockSize, Point1[1] : Point1[1] + BlockSize]
    ).astype(np.float64)

    Block2 = (
        img[Point2[0] : Point2[0] + BlockSize, Point2[1] : Point2[1] + BlockSize]
    ).astype(np.float64)
...
def Step2_3DFiltering(BlockGroup_basic, BlockGroup_noisy):
    ...
    if Weight > 0:
        WienerWeight = 1.0 / (sigma**2 * Weight)
...
def Step2_Aggregation(
    BlockGroup_noisy, WienerWeight, BlockPos, finalImg, finalWeight, finalKaiser
):
    """
    ...
    for i in range(BlockPos.shape[0]):
        finalImg[
            BlockPos[i, 0] : BlockPos[i, 0] + BlockGroup_noisy.shape[1],
