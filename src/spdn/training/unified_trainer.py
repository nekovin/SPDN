"""
Unified training framework for SPDN models.

This module provides a consistent training interface for all SPDN models
...
class UnifiedTrainer:
    """Unified trainer for SPDN models."""
    ...
    def _setup_training(self):
        ...
        self.model = ModelFactory.create_model(
            self.config.model_name, self.config.model_config
        )
    ...
    def train(self):
        ...
        print(
            f"Dataset: {len(self.train_loader.dataset)} train, {len(self.val_loader.dataset)} val"
        )
    ...
    def _compute_loss(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if self.config.loss_function == "spdn":
            # For SPDN loss, we need flow and noise components
    ...
    def _save_checkpoint(self, epoch: int, is_best: bool = False):
        ...
        checkpoint = {
            ...
            "loss": self.training_history["train_loss"][-1]
        ...
        checkpoint_path = os.path.join(
            self.config.checkpoint_dir, f"{self.config.model_name}_epoch_{epoch}.pth"
        )
        ...
        if is_best:
            best_path = os.path.join(
                self.config.checkpoint_dir, f"{self.config.model_name}_best.pth"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train SPDN model")
    parser.add_argument(
