"""
Model factory for SPDN framework.
...
from .spdn.spdn_attention import SPDNAttention
from .spdn.spdn_attention_small import SmallSPDNAttention
from .spdn.spdn_no_attention import SPDNNoAttention
...
class SPDNModelWrapper(BaseModel):
    """Wrapper for SPDN models to provide consistent interface."""

    def __init__(self, model_class: Type[nn.Module], config: ModelConfig):
        """Initialize SPDN model wrapper.

        Args:
            model_class: SPDN model class
            config: Model configuration
        """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass through SPDN model.
...
class ModelFactory:
    ...
    _models = {
        # SPDN models
        "spdn_attention": (SPDNAttention, SPDNModelWrapper),
        "spdn_attention_small": (SmallSPDNAttention, SPDNModelWrapper),
        "spdn_no_attention": (SPDNNoAttention, SPDNModelWrapper),
        # U-Net models
    ...
    _default_configs = {
        ...
        "simple_unet": {"n_channels": 1, "n_classes": 1, "bilinear": True},
    }
...
def load_model_from_checkpoint(
    checkpoint_path: str, model_name: str, config: Optional[Dict[str, Any]] = None
) -> BaseModel:
