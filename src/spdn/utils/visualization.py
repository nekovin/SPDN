"""
Visualization utilities for SPDN training and evaluation.
...
class TrainingVisualizer:
    ...
    def plot_training_curves(
        ...
        if val_losses and len(val_losses) > 0:
            ...
            axes[0].plot(
                val_epochs, val_losses, "r-", label="Validation Loss", linewidth=2
            )
    ...
    def plot_model_predictions(
        ...
        if save_path:
            ...
            plt.savefig(
                self.output_dir / "model_predictions.png", dpi=300, bbox_inches="tight"
            )

        plt.close()

    def plot_spdn_separation(
        self,
        input_images: torch.Tensor,
        ...
        """Plot SPDN flow and noise separation results.
        ...
        flow_target_np = (
            self._tensor_to_numpy(flow_targets) if flow_targets is not None else None
        )
        noise_target_np = (
            self._tensor_to_numpy(noise_targets) if noise_targets is not None else None
        )
        ...
        if save_path:
            ...
            plt.savefig(
                self.output_dir / "spdn_separation.png", dpi=300, bbox_inches="tight"
            )

        plt.close()

    def plot_metrics_comparison(
        self, metrics_dict: Dict[str, List[float]], save_path: str = None
    ) -> None:
        ...
        if save_path:
            ...
            plt.savefig(
                self.output_dir / "metrics_comparison.png", dpi=300, bbox_inches="tight"
            )
    ...
    def plot_attention_maps(
        ...
        if save_path:
            ...
            plt.savefig(
                self.output_dir / "attention_maps.png", dpi=300, bbox_inches="tight"
            )

        plt.close()

    def plot_loss_components(
        self, loss_dict: Dict[str, List[float]], save_path: str = None
    ) -> None:
        ...
        if save_path:
            ...
            plt.savefig(
                self.output_dir / "loss_components.png", dpi=300, bbox_inches="tight"
            )
    ...
    def create_training_report(
        ...
        if save_path:
            ...
            plt.savefig(
                self.output_dir / "training_report.png", dpi=300, bbox_inches="tight"
            )
    ...
    def save_metrics_json(
        self, metrics_dict: Dict[str, Any], filename: str = "metrics.json"
    ):
