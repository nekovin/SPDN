"""
Metrics calculation utilities for SPDN evaluation.
...
class MetricsCalculator:
    """Comprehensive metrics calculator for SPDN evaluation."""
    ...
    def calculate_all_metrics(
        ...
        # Structural metrics
        metrics["structural_similarity"] = self.calculate_structural_similarity(
            pred_np, target_np
        )
        ...
        # Contrast metrics
        metrics["contrast_preservation"] = self.calculate_contrast_preservation(
            pred_np, target_np
        )
        ...
        if input_image is not None:
            ...
            metrics["noise_reduction"] = self.calculate_noise_reduction(
                input_np, pred_np, target_np
            )
            metrics["snr_improvement"] = self.calculate_snr_improvement(
                input_np, pred_np
            )

        # Vessel-specific metrics (for OCT/OCTA)
        metrics["vessel_continuity"] = self.calculate_vessel_continuity(
            pred_np, target_np
        )
    ...
    def calculate_structural_similarity(
        ...
        """
        # Normalize images
        pred_norm = (prediction - prediction.min()) / (
    ...
    def calculate_contrast_preservation(
        self, prediction: np.ndarray, target: np.ndarray
    ) -> float:
        ...
        correlation = np.corrcoef(pred_contrast.flatten(), target_contrast.flatten())[
            0, 1
        ]
    ...
    def calculate_noise_reduction(
        self, input_image: np.ndarray, prediction: np.ndarray, target: np.ndarray
    ) -> float:
    ...
    def calculate_snr_improvement(
        self, input_image: np.ndarray, prediction: np.ndarray
    ) -> float:
    ...
    def calculate_vessel_continuity(
        self, prediction: np.ndarray, target: np.ndarray
    ) -> float:
        ...
        self._get_connected_components(pred_binary)
        self._get_connected_components(target_binary)
    ...
    def calculate_vessel_contrast(
        ...
        pred_contrast = np.mean(prediction[pred_vessels]) - np.mean(
            prediction[pred_background]
        )
        target_contrast = np.mean(target[target_vessels]) - np.mean(
