#!/usr/bin/env python3
"""
Unified SPDN training script.

This script provides a clean, simple interface for training SPDN models
...
from spdn.training.unified_trainer import UnifiedTrainer, TrainingConfig, load_config_from_yaml
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train SPDN model with unified framework"
    )
    parser.add_argument("--config", type=str, help="Path to configuration YAML file")
    parser.add_argument(
        "--model",
        type=str,
        default="spdn_attention",
        choices=["spdn_attention", "spdn_attention_small", "spdn_no_attention"],
    ...
    parser.add_argument(
        "--dataset-root", type=str, help="Root directory of the dataset"
    )
    parser.add_argument(
        "--checkpoint-dir",
        type=str,
    ...
    if not config.dataset_root or not os.path.exists(config.dataset_root):
        ...
        print(
            "Please set DATASET_DIR_PATH environment variable or use --dataset-root argument."
        )
        sys.exit(1)

    # Print configuration summary
    print("\n" + "=" * 50)
    print("SPDN Training Configuration")
    print("=" * 50)
